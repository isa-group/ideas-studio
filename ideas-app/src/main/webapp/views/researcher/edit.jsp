	...
	jQuery(function() {
		...
		if(localStorage.getItem("social") != ""){
			$('#userAccount.username').prop('disabled', true);
 			...
 			if(avatar.size() != 0)
				$('#settingsButtonPanel').append("<span class='lead ' style='float: left; color: #628FB8;'>Information acquired from social login</span>");
		}
...
// 			$("#statusPanel").append('<spring:message code="researcher.settings.problems" />');
			...
			if (repeatedEmail == "true") {
				$("#statusPanel").append('<spring:message code="security.repeatedEmail.error" />');
			} else {
				$("#statusPanel").append('<spring:message code="researcher.settings.problems" />');
				if(avatar.size() == 0){
...
<div id="loginOKPanel" class="modal" data-backdrop="static" data-keyboard="false">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        <button type="button" class="btn btn-primary" onclick="redirecToLogin()">Return to Login</button>
      </div>
    </div><!-- /.modal-content -->
  </div><!-- /.modal-dialog -->
</div><!-- /.modal -->
...
<div id="loginFailPanel" class="modal" data-backdrop="static" data-keyboard="false">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
      ...
      <div class="modal-footer">
        <button type="button" class="btn" onclick="redirecToLogin()">Return to Login</button>
      </div>
    </div><!-- /.modal-content -->
  </div><!-- /.modal-dialog -->
</div><!-- /.modal -->
...
<div id="settingsPageHeader">
	...
	<security:authorize access="isAuthenticated()"><h2><spring:message code="settings.title.edit" /></h2></security:authorize>
	<security:authorize access="isAnonymous()"><h2><spring:message code="settings.title.new" /></h2></security:authorize>
...
<div id="settingsContainer">
	<ul class="nav nav-tabs" id="settingTabs">
		...
		<security:authorize access="isAuthenticated()">
			...
			<li><a href="#socialTabContent" data-toggle="tab" id="socialTab"><spring:message code="socialnetwork.editHeader" /></a></li>
		</security:authorize>
	</ul>
		...
		<div class="tab-content">
			...
			<security:authorize access="isAuthenticated()">
				...
				<div class="tab-pane fade active" id="accountTabContent">
					...
					<div class="control-group">
						<div class="input-group">
							<span class="input-group-addon"><spring:message code="security.username" /></span>
							<form:input path="userAccount.username" placeholder="myusername" type="text" class="form-control" />
						</div>
						<span class="label label-danger"><form:errors path="userAccount.username" /></span>
					</div>


					<div class="control-group">
						<div class="input-group">
							<span class="input-group-addon"><spring:message code="security.oldpassword" /></span>
							<input type="password" id="oldPass" name="oldPass" class="form-control" >
						</div>
						<span id="badPassError" class="label label-danger hide alert-error"><spring:message code="security.badpassword.error" /></span>
						
					</div>

					<div class="control-group">
						<div class="input-group">
							<span class="input-group-addon"><spring:message code="security.newpassword" /></span>
<!-- 							<input type="password" id="mypass" name="mypass" class="form-control" > -->
<!-- 							<input type="password" id="mypass" name="mypass" class="form-control" > -->
							<form:password id="mypass" name="mypass" class="form-control" path="userAccount.password" />
						</div>
						<span class="label label-danger"><form:errors path="userAccount.password" /></span>
						
					</div>

					<div class="control-group">
						<div class="input-group">
						<span class="input-group-addon"><spring:message code="security.repeatpassword" /></span>
<%-- 						<form:password id="repeatPass" name="repeatPass" class="form-control" path="userAccount.repeatPassword" /> --%>
						...
						<span id="repeatPassError" class="label label-danger hide alert-error"><spring:message code="security.differentpasswords.error" /></span>
					...
					<div>
						<jstl:forEach items="${missingServices}" var="snetwork">
							<div id="${snetwork}-connections">
								<div id="connect-${snetwork}" class="btn btn-primary ${snetwork}-icon connection" onclick='location.href="connect/${snetwork}"'>Connect to ${snetwork}
								</div>
								<hr>
							</div>
						</jstl:forEach>
