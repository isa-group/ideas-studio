...
//			_this.preventBackspaceKeyPress();
    ...
    setInspectorModelContent : function () {
        ...
        modelInspectorContentWrapper.html(content);
				...
				// Establish a temporary id for input elements in order to re-focus them after modification
				var i = 0;
				modelInspectorContentWrapper.find("input[ng-model]").each(function () {
					// Check if input will be generated by ng-repeat
					// debugger;
	...
	progressBar : {
		...
		_selectBindingTextNote : {
															...
															function() {
																...
																$(
																		...
																		.prop(
																				...
																				false);
																$(
																		"#selectBindingType option:disabled")
																		.each(
																				function() {
																					$(
																							this)
																...
																$(
																		"#selectBindingAttributeType")
																		.prop(
																				"disabled",
																				false);
																$(
																		"#selectBindingAttributeType option:disabled")
																		.each(
																				function() {
																					$(
																							this)
						...
						.change(
								function() {
									...
									if (value === "newElement") {
										...
										$("#selectBindingAttributeType").prop(
												"disabled", false);
										$(
												"#selectBindingAttributeType option:disabled")
												.each(
										...
										var $ancestorBinding = _this
												.getStep2AncestorBinding();

										if ($ancestorBinding != null) {
											$("#selectBindingType").val("div"); // watch
											// out
											// if
											// "div"
											// exists
											...
											// element
											_this
													.disableListOfOptions(
															$("#selectBindingAttributeType"),
															_this
																	.getRDFaDeclaredElements($ancestorBinding));
										} else {
			...
			disableListOfOptions : function($select, listOfOptions) {
				...
				if ($select.prop("tagName").toLowerCase() === "select") {
					...
					if (disabledItems_i == $select.find("option").length) {
						...
						$("#selectBindingAttributeType").val(
								$("#selectBindingAttributeType option:enabled")
										.first().val())
					}
				} else {
			...
			getStep2AncestorBinding : function() {
				...
				var binding = DescriptionInspector.vars.progressBar.step2, ancestorElement = binding.commonAncestorContainer.parentElement, ret = null;

				if (ancestorElement != null) {
					var $ancestor = $(ancestorElement);
					if (!$ancestor.hasClass("ideas-binding")
							|| $ancestor.prop("tagName").toLowerCase() != "div") {
			...
			stepForwardManager : function() {
				...
				$(el).addClass("ideas-binding highlighted-fragment").attr({
				...
				// RDFa properties
				if (this.isRDFa()) {
					var rdfaElementType = $("#selectBindingType"), rdfaElementValue = $("#typeValue"), rdfaAttributeType = $("#selectBindingAttributeType"), rdfaAttributeValue = $("#attributeTypeValue");
	...
	removeBindingInspector : {
		...
		_highlightBindingsToRemove : function(callback) {
			...
			$("#inspectorContent .ideas-binding").addClass(
					"to-remove highlighted-fragment").removeClass(
					"ideas-binding");
		...
		_showModalRemoveBinding : function($obj) {
			...
			var bind = Binding.getBinding($obj), rowAux = parseInt(bind.startRows[0]) + 1, content = "<label>Source:</label> <span>"
					+ bind.textNote
					+ "</span> <br/> \
						   <label>Target:</label> <span>"
					+ bind.getTargetText()
					+ "</span> <br/> \
						   <label>Line:</label> <span>"
					+ rowAux + "</span>", ace = DescriptionInspector.ace, _this = this;
				...
				$(".modal input.to-bulkremove:checked").each(function() {
					...
					$("#inspectorContent .to-remove").each(function(index) {
						if (index == $($this).data("id-remover")) {
							...
							$bindingMap.removeBinding(bindingObj); // FIXME:
							// remove in
							// the
							// future.
						}
		...
		setDefaultContent : function() {
			...
			$("#inspectorContent .to-remove").unbind("click").click(function() {
	...
	descriptionFormatView : {
		...
		formatTab : {
			...
			onClick : function() {
				...
				var fullView = DescriptionInspector.descriptionFormatView, _this = this;
	...
	loaders : {
		...
		descriptionFileCreator : function(inspectorLoader) {
			...
			$this
					.css("margin-left", "-".concat(($this.width() / 2)
							.toString()));
			$this
					.css("margin-top", "-".concat(($this.height() / 2)
							.toString()));

			$this
					.click(function() {

						var fileUri = DescriptionInspector
								.getCurrentDescriptionFileUri(), fileName = fileUri
								.split("/")[2].split(".")[0], fileContent = '<!doctype html>\n<html>\n\t<body>\n\t\t<h4>Description</h4>\n\t</body>\n</html>';

						EditorManager.createNode(fileUri, fileName, ".html",
								function() {
									FileApi.saveFileContents(fileUri,
											fileContent, function() {
												currentSelectedNode.getParent()
														.sortChildren();
	...
	scrollInspectorToBinding : function($htmlObj, click) {
		if (DescriptionInspector.existBinding()) {
			// isVisible
			var topView = $("#inspectorContent").scrollTop(), topElement = $htmlObj
					.position().top, value = topView + topElement - 50, time = Math
					.abs(1500 - value * 0.4);

			if (time > 2000)
				time = 1500; // if the inspectorContent is too large.
	...
	highlightAllBindings : function() {
		...
		if (DescriptionInspector.existBinding()) {
			...
			if (DescriptionInspector.vars.mode == "editor"
				...
				if (ranges.length > 0)
					DescriptionInspector
							.scrollEditorToRange(ranges[ranges.length - 1]);
