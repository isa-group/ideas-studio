...
/**
 * Contiene todos los métodos necesarios para el funcionamiento de IDEAS-Binding.
 * Se ha estructurado el código de la siguiente forma:
 * 	   - en la raíz están los métodos comunes a las distintas funcionalidades de IDEAS-Binding.
 * 	   - los métodos "privados" empiezan con "_", y servirán para recordar al desarrollador que dicho método es llamado a partir de un otro, y nunca forma directa.
 * 	   - dividiéndolo en 4 bloques:
 *         - progressBar (menú de "add"): este a su vez se subdivide en los distintos pasos (steps) de la barra de progreso. Hay 3 steps.
 *         - removeBindingInspector (menú de "remove").
 * 	       - loaders: necesario para darle funcionamiento a las llamadas del usuario ("onClick", "onFocus", ...).
 * 	       - ace: simplifican la llamadas los métodos del editor web.
 * 	       - inspectorContent: centraliza el funcionamiento y método de gesitón del contenido del inspector de descripción.
...
var DescriptionInspector = {
	...
	vars : {
		...
		/**
		 * Variable usada para almacenar el contenido del inspector cuando haga falta.
		 * @memberof DescriptionInspector.vars
		 */
		inspectorContent : "",

		/**
		 * Variable que contiene los selectores más usados del DescriptionInspector.
		 * @memberof DescriptionInspector.vars
		...
		/**
		 * Variables para el funcionamiento del progressBar. Almacenan la información conseguida en cada step.
		 * @memberof DescriptionInspector.vars
		...
		/**
		 * Controla si la modificación realizada sobre el editor es realizada desde un método o desde el mismo editor.
		 * 0 indica que se realiza sobre el editor
		 * 1 indica que se realiza fuera del editor, por ejemplo la llamada al método "DescriptionInspector.modifyBindings".
		 * @memberof DescriptionInspector.vars
		 */
		modificationFlag : 0
	...
	/**
	 * Método de inicio de DescriptionInspector. Busca un fichero descriptor asociado al fichero actualmente abierto.
	 * Si existe un fichero descriptor, carga todos los métodos loaders de inicio. En caso contrario, muestra botón de
	 * construcción del fichero descriptor con una plantilla.
	 * @this window
	 * @memberof DescriptionInspector
	 * @param {jQuery} inspectorLoader - Contenedor jQuery del inspector  
	 */
	loader : function (inspectorLoader) {
		var _this = DescriptionInspector;
		
		if ( _this.existCurrentDescriptionFile() ) {
			_this.loadInspectorContent(inspectorLoader, _this.getDescriptionFileContent());
	...
	existCurrentDescriptionFile : function () {
		return getNodeByFileUri(this.getCurrentDescriptionFileUri()) ? true:false;
	},
	...
	/**
	 * Elimina toda la estructura del inspector dejándolo en su aspecto inicial al cargar la aplicación.
	 * Se utiliza cuando ya no queda ningún fichero que mostrar.
	 * @memberof DescriptionInspector 
	 */
	empty : function () {
	...
	/**
	 * Muestra u oculta el menú expandible en función de la extensión del fileUri pasado como parámetro.
	 * Si se trata de un fichero descriptor, oculta el menú. En caso contrario, lo muestra.
	 * @memberof DescriptionInspector
	 */
	...
	/**
	 * Añade la pestaña de inspector del módulo y le da funcionalidad al hacerle clic.
	 * Sirve para mostrar el contenido del inspector creado en el manifiesto del módulo en cuestión.
	 * @memberof DescriptionInspector
	...
	/**
	 * Almacena el contenido del htmlObj pasado por parámetro en el fichero descriptor asociado.
	 * @memberof DescriptionInspector
	 */
	saveContentToDescriptionFile : function (content) {
		...
		if (content) {
			...
			// Session content
			if (fileUri in EditorManager.sessionsMap) EditorManager.sessionsMap[fileUri].getBaseSession().setValue(content);
	...
	/**
	 * Almacena en servidor el contenido actual del inspector de descripción.
	 * Es necesario comprobar que el fichero descriptor en cuestión se encuentra cerrado antes de intentar guardarlo.
	 * @memberof DescriptionInspector
	 */
	...
	/**
	 * Prepara el contenido a almacenar en el fichero de descripción correspondiente.
	 * @memberof DescriptionInspector
	 */
	...
	/**
	 * Obtiene la uri del fichero descriptor que hace referencia al fichero actualmenente abierto.
	 * @memberof DescriptionInspector
	...
	getCurrentDescriptionFileUri : function () {
		...
		var arrayCurrentUri = EditorManager.getCurrentUri().split('/')
			, retFileUri = "";
		
		if (arrayCurrentUri.length == 3) {
			var fileName = arrayCurrentUri[2].split(".")[0];
			retFileUri = arrayCurrentUri[0] +"/"+ arrayCurrentUri[1] +"/"+ fileName +".html";
		}
	...
	getDescriptionFileContent : function () {
		var urlReq = 'file/getFileContent?fileUri=' +this.getCurrentDescriptionFileUri()
			, descriptionData = "";
		...
		$.ajax({
			...
			success: function(content) {
				if (content) {
					...
					//console.log('inspector was successfully downloaded description file data');
				} else {
	...
	/**
	 * Objeto inspectorContent centralizador de operaciones comúnes sobre esta sección de IDEAS.
	 * Dicha sección se refiere al contenido del inspector de descripción.
	 * @memberof DescriptionInspector
	...
	inspectorContent: {
		...
		height: function () {
			var inspectorHeight 	=	$("#editorInspector").height()
				...
				, expandMenuHeight 	= 	$("#expandableMenu").css("display") == "none" ? 
											0 : $("#expandableMenu").height()
				;
		...
		/**
		 * Redimensiona el tamaño del inspectorContent teniendo en cuenta el zoom actual.
		 * @memberof DescriptionInspector.inspectorContent
		 * @returns {boolean} Devuelve true si se han cambiado los valores, y false si no.
		 */
		resize : function () {
			var _this  			= DescriptionInspector.inspectorContent
				, $inspector 	= _this.getSelector()
				, zoom   		= 1/$inspector.css("zoom")
				, width  		= _this.width()
				, height 		= _this.height()
				;
		...
		/**
		 * Almacena en una variable "inspectorContent" el contenido actual del inspector de descripción.
		 * Sirve para que tengamos un respaldo del contenido si hemos modificado algo y queremos cancelar el cambio.
		 * @memberof DescriptionInspector.inspectorContent
		 */
		saveCurrentLocalData : function () {
			DescriptionInspector.vars.inspectorContent = this.getSelector().html();
		},
		...
		loadLocalContent : function () {
			if (DescriptionInspector.vars.progressBar.step3 != null)
				this.getSelector().html(DescriptionInspector.vars.progressBar.step3);
		}
		
	},
	
	/**
	 * Expande el menú desplegable independientemente del tipo que sea (add o remove).
	 * Define la animación de despliegue.
	 * @memberof DescriptionInspector
	 */
	expandInspectorMenu : function (menu) {
		...
		expandableMenu.animate({height:"0px"},100, function () {
    ...
    /**
     * Contrae el menú desplegable independientemente del tipo que sea (add o remove).
     * Define la animación de contracción.
     * @memberof DescriptionInspector
     */
    unexpandInspectorMenu : function (callback) {
    	...
    	expandableMenu.animate({height:"0px"}, 200, function () {
			expandableMenu.css("background-color", "#fafafa");
    		$(".expanded")
    			.removeClass("expanded")
    			.css("display", "none");

    		expandableMenu.animate({height:"33px"}, 500);
    ...
    /**
     * Muestra el contenido del inspector de descripción haciendo clic en su pestaña.
     * Además trata de redimensionar el contenedor "inspectorContent" para visualizarlo bien.
     * @memberof DescriptionInspector
     */
	...
	/**
	 * Obtiene el texto de anotación a partir de un objeto HTML. Si el objeto no cuenta con un atributo data-text,
	 * se coge como anotación el texto/valor de dicho objeto HTML.
	 * @param {HTMLObject}
	 * @memberof DescriptionInspector
	...
	/**
	 * Carga sobre el inspector de descripción el contenido pasado por parámetro, cargando antes la plantilla html.
	 * @param {jQuery} loader
	 * @param {string} content - Cadena del texto html.
	 * @memberof DescriptionInspector
	 */
	loadInspectorContent : function (loader, content) {
		if (loader && content) {
			...
			console.error("::loadInspectorContent undefined params", loader, content);
		}
	},
	...
	ace : {
		 ...
		 */
		getEditor 	 : function () { return document.editor; },
		 ...
		 */
		getSession 	 : function () { return (document.editor) ? this.getEditor().getSession() : null; },
		 ...
		 */
		getSelection : function () { return (document.editor) ? this.getEditor().getSelection() : null; },
	...
	expandableMenu : {
		 ...
		 * @param {string} text - Cadena de texto del título.
		 * @returns {text}
		 */
		 ...
		 * @param {string} text - Cadena de texto de la descripción.
		 * @returns {boolean}
		 */
		setDescription : function (text) {
			return this.getSelector().find(".contentWrapper > p").first().html(text);
		}
	},
	...
	progressBar : {
		 ...
		 * @memberof DescriptionInspector.progressBar
		 * @param {string} Título del progressBar
		 */
		...
		/**
		 * Modifica la descripción del porgressBar por un valor pasado por parámetro.
		 * @memberof DescriptionInspector.progressBar
		 * @param {string} Descripción del progressBar
		 */		
		...
		/**
		 * Añade referencia de texto del fragmento seleccionado pasado como parámetro.
		 * @memberof DescriptionInspector.progressBar
		 * @param {string} fragmentText

		 */
		appendSelectedFragment : function (fragmentText) {
			var $progressContent 	= $("#descriptionInspectorHeaderAdd .contentWrapper p.progressContent")
				, prevContent 		= $progressContent.html()
				, labelName 		= ($progressContent.find("label").length > 0) ? "Desc.: " : "Code: "
				, escFragText 		= fragmentText.replace(/"/g, "&quot;").replace(/\t/g, " ");
			var actualContent = prevContent +
					'<div class="container-fluid"><div class="row" style="margin-bottom:5px;">'+
						...
						'<input class="selectedFragment" type="text" '+
								'value="'+ escFragText +'" style="pointer-events: none; cursor: default;" disabled /></div>'+
					'</div></div>';
		...
		/**
		 * Comprime el texto del fragmento para que se visualice correctamente en el menú de progressBar.
		 * @memberof DescriptionInspector.progressBar
		 */
		compressFragmentTexts : function () {
			
			$("input.selectedFragment").each(function () {
				var fragText 		= $(this).val()
					, fragSize 		= fragText.length
					, inputSize 	= $(this).width()/8
					, exceedsView 	= fragSize > inputSize
					;
		...
		setDefaultContent : function () {
			...
			editorInspector.find(".expanded footer .btn-primary").addClass("inactive");
			editorInspector.find(".highlighted-fragment").removeClass("highlighted-fragment");
		...
		/**
		 * Da comienzo al funcionamiento del progressBar y almacena el contenido del inspector antes de modificarlo.
		 * @memberof DescriptionInspector.progressBar
		 */
		start : function () {
		...
		/**
		 * Cancela el progressBar y establece los valores por defectos para la próxima vez que se vaya a ejecutar.
		 * @memberof DescriptionInspector.progressBar
		 * @param {function} Función opcional a ejecutarse una vez se cancele el progressBar  
		 */		
		...
		/**
		 * Termina satisfactoriamente la ejecución del progressBar, guardando el contenido final del "inspectorContent".
		 * @memberof DescriptionInspector.progressBar
		 */		
		finish : function () {
		...
		_selectAceText : {
			...
			run : function () {
				...
				$(".ace_content").bind("click", function (event) {
					if ( step1.isValid() ) {
						//console.log("selected text:", DescriptionInspector.ace.getEditor().getSelectedText());
				...
				$(document).bind("keyup", function () { $(".ace_content").click(); });
			},
			...
			/**
			 * Almacena contenido relevante al step 1. En este caso sólo se almacena "DescriptionInspector.vars.progressBar.step1"
			...
			_saveSelectedContent : function () {
				...
				var $editor = DescriptionInspector.ace.getEditor()
					, currentSelectionRange = $editor.getSelection().getRange(); 
				...
				progress.appendSelectedFragment(DescriptionInspector.vars.progressBar.step1.textNote);
			...
			isValid : function () {
				return this._isActive() && 
				       DescriptionInspector.ace.getEditor().getSelectedText().replace(/\s+/g, "") != "";
			},
			
			/**
			 * Establece el contenido por defecto del step 1. Siempre que se cargue este step,
			 * se cargará esta configuración.
			 * @memberof DescriptionInspector.progressBar.step1
			 */
			setStepDefaultContent : function () {
				...
				$("#editorInspector .expanded .one")						// Desactiva numero de progreso step 1 y márcalo como exitoso
					.addClass("active")
					.removeClass("no-color")
					.html('<span style="color:#F5F5F5;">1</span>');
				...
				$("#editorInspector .expanded .three")
					.addClass("no-color")
					.removeClass("active")
					.html('3');
...
//				DescriptionInspector.progressBar.setTitle("Description select");
				DescriptionInspector.progressBar.setDescription("Please select code fragment in the editor on the left and press next");
			},
		...
		_selectInspectorText : {
			...
			/**
			 * Almacena contenido relavante al step 2. En este caso sólo se alamacena "DescriptionInspector.vars.progressBar.step2".
			 * @memberof DescriptionInspector.progressBar.step2
			 */
			_saveSelectedContent : function () {
				...
				if (window.getSelection) {
					var range = window.getSelection().getRangeAt(0)
						, progress = DescriptionInspector.progressBar;
					...
					//console.log("DescriptionInspector.vars.progressBar.step2 saved:", DescriptionInspector.vars.progressBar.step2);
			 ...
			 */
			isValid : function () {
				return DescriptionInspector.vars.progressBar.step1 && this._isActive();
			},
			
			/**
			 * Establece el contenido por defecto del step 2. Siempre que se cargue el step se cargará esta configuración.
			 * @memberof DescriptionInspector.progressBar.step2
			 */
			setStepDefaultContent : function () {
				...
				// Style
				expandedMenu.find("#descriptionInspectorHeader .progress-bar")		// Modifica barra de progreso a step 2
					.css("width","51%");
			
				expandedMenu.find(".one")												// Desactiva numero de progreso step 1 y márcalo como exitoso
					.removeClass("active")
					.html('<span class="glyphicon glyphicon-ok" style="color:#F5F5F5;margin-left:-4px;"></span>');
				
				expandedMenu.find(".two")												// Activa el numero de progreso step 2
					.removeClass("no-color")
					.addClass("active")
					.html('<span style="color:#F5F5F5;">2</span>');
				
				expandedMenu.find(".three")											// Asegura que step 3 está desactivado
					.removeClass("active")
					.html('3');
			 ...
			 */
			setDefaultHeaderText : function () {
//				DescriptionInspector.progressBar.setTitle("Inspector select");
				var progress = DescriptionInspector.progressBar;

				progress.setDescription("");
				progress.appendSelectedFragment(DescriptionInspector.vars.progressBar.step1.textNote);
				progress.setDescription(
					progress.getDescriptionSelector().html() +
			...
			stepFowardManager : function () {
				...
				$("#inspectorContent").bind("click", function () {
					...
					$(document)
						.unbind("keyup")
						.bind("keyup", function() { updateDescriptionSelectedFragmentText() });
				});
		...
		_selectBindingTextNote : {
			...
			isValid : function () {
				return DescriptionInspector.vars.progressBar.step1 && 
					   DescriptionInspector.vars.progressBar.step2 && this._isActive();
			},
			 ...
			 * @memberof DescriptionInspector.progressBar.step3
			 */			
			onCancel : function () { DescriptionInspector.progressBar.cancel(); },

			/**
			 * Establece el contenido por defecto del step 3. Siempre que se cargue el step se cargará esta configuración.
			 * @memberof DescriptionInspector.progressBar.step3
			 */			
			setStepDefaultContent : function () {
	...
	removeBindingInspector : {
		...
		/**
		 * Muestra ventana modal para remover un bindign en concreto. Se muestran los parámetros básicos del binding seleccionado.
		 * @memberof DescriptionInspector.removeBindingInspector
		 * @param {jQuery} $obj
		 */
		...
		/**
		 * Muestra la ventana modal para realizar "Bulk remove" listando todos los bindings posibles para eliminar.
		 * @memberof DescriptionInspector.removeBindingInspector
		 */
		_showModalBulkRemoveBindings : function () {
			console.log("Showing binding bulk delete modal window.");

			var binds = $("#inspectorContent .ideas-binding")
				, $listContainer = $("<div>")
				, _this = this;
			...
			$("#inspectorContent .to-remove").each(function(index) {
				...
				$listContainer.append("<div title='"+ bindObj.startRows[0] +": "+ bindObj.getTargetText() +"'><input id='bulkremove"+ index +"' class='to-bulkremove' name='checkboxBulkRemover' data-id-remover='"+ index +"' type='checkbox' />"+ 
											"<label for='bulkremove"+ index +"'>"+ bindObj.textNote +"</label></div>");
			});

			// Modal content
			var title = "Delete bindings",
				primaryText = "OK",
				content = ($listContainer.find("div").length > 0) ? 
						   '<div class="bulkRemover">'+ 
						   		$listContainer.html() +
			...
			showModal(
				title, content, primaryText, 
				function () {

					$(".modal input.to-bulkremove:checked").each(function () {
						var $this = this
							, hasBindingToRemove = true;
						$("#inspectorContent .to-remove").each(function(index) {
							if (index == $($this).data("id-remover")) {
								var bindingObj = Binding.getBinding( $(this) );
								$(this).addClass("to-bulkremove");
								$bindingMap.removeBinding(bindingObj);				//FIXME: remove in the future.
							}
						});
					});

					_this.unwrap(
						$("#inspectorContent .to-bulkremove").unbind("click")
					);
		...
		/**
		 * Establece el contenido por defecto para el menú de eliminación de bindings.
		 * @memberof DescriptionInspector.removeBindingInspector
		 */
		setDefaultContent : function () {
			...
			$("#descriptionInspectorHeaderRemove .proceedAction")
		...
		/**
		 * Muestra un mensaje u otro en el description si todavía quedan bindings en el inspector.
		 * @memberof DescriptionInspector.removeBindingInspector
		 */
		loadAvailableBindingsToRemoveMessage : function () {
			...
			if (DescriptionInspector.existBinding()) {
				DescriptionInspector.expandableMenu.setDescription(
					'Please select a binding on the text below or press "<u>Bulk delete</u>" to remove more than one binding at one time.'
				);
			} else {
				DescriptionInspector.expandableMenu.setDescription(
					'There is no binding to remove on the text below.'
				);
			}
		},
	...
	descriptionFormatView : {
		...
		/**
		 * Inicializa todo el ecosistema de descriptionFormatView y su contenido.
		...
		setDefaultStructure : function () {
			if (this.getHtmlObj().length === 0) {
				$("#editorWrapper").append('<div id="ideasBinding-descriptionBoardContent"></div>');
			}
		},
		...
		setDefaultContent : function () {
			...
			if (content) {
				...
				content = content.replace(/(ideas-binding)/g,"ideas-binding highlighted-fragment");
		...
		/**
		 * Comprueba si el estamos en la situación de mostrar el DescriptionFullView: el inspector está oculto
		 * @memberof DescriptionInspector.descriptionFormatView
		 */
		mayShow : function () {
		...
		show : function () {
			...
			if (!EditorManager.currentDocumentHasProblems()) {
				...
				if (this.mayShow()) {
					...
					setTimeout(function () {
						EditorManager.changeFormat(EditorManager.currentUri, $("#editorFormats li.formatTab").first().text().toLowerCase());
					},1);
				...
				CommandApi.echo('<span style="color:red;">Can not show desc main view while errors exist.</span>');
			}
		...
		hide : function () {
			...
			this.getHtmlObj()
				.fadeOut(function () {
					...
					if ( $("#editorFormats .formatTab.active").text().toLowerCase() === "desc" ) {
		...
		customEvents : function () {
			...
			/*inspector.bind("inspectorChangeState", function (e, open) {
				if (open) {
					_this.hide();
				}
			});*/
				...
				$("#ideasBinding-descriptionBoardContent .ideas-binding")
					...
					.dblclick(function () {
						...
						var content = '<div class="container-fluid">'+
										  '<div class="row"><div class="col-xs-3"><label style="color: #aaa;">Original value:</label></div><div class="col-xs-9"><span id="ideasBindingOriginalBindingToModify" style="color: #aaa;">'+ binding.getTargetText() +'</span></div></div>'+
										  '<br/><div class="row"><div class="col-xs-3"><label>New value:</label></div> '+
										  '<div class="col-xs-9"><input type="text" id="ideasBindingValueToModify" value="'+ $(this).text() +'"'+
						...
						showModal(
							...
							function () {
								...
								var content = DescriptionInspector.descriptionFormatView.getHtmlObj().html()
									, preparedContent = DescriptionInspector.prepareDescriptionContentToSave(content)
								
								if (preparedContent != null) {
									DescriptionInspector.saveContentToDescriptionFile(preparedContent);
									$("#inspectorContent").html(preparedContent);
									DescriptionInspector.loaders.bindings();
									CommandApi.echo("New content saved.");
								}

								//TODO: update description inspector
		...
		formatTab : {
			...
			/**
			 * Obtiene la lista de los demás formatos disponibles en lista de objetos html.
			 */
			...
			onClick : function () {
				...
				var fullView = DescriptionInspector.descriptionFormatView
					, _this  = this;
	...
	loaders : {
		...
		customEvents : function () {
			...
			inspector.bind("inspectorChangeState", function (e, open) {
				if (open) {
					if (document.editor) DescriptionInspector.highlightAllBindings();

					if (DescriptionInspector.descriptionFormatView.getHtmlObj().length > 0)
						DescriptionInspector.descriptionFormatView.hide();
					
				} else {
					if (document.editor) DescriptionInspector.unHighlightAllBidings();
				}
			});
		...
		/**
		 * Código a ejecutar cuando se realiza una modificación del contenido del editor.
		 * @memberof DescriptionInspector.loaders
		 */
		onEditorChangeBindingValue : function () {
			
			DescriptionInspector.ace.getEditor().on("change", function(e) {
				...
				if (DescriptionInspector.vars.mode === "editor" && 
					...
					if ( !$("#appGenericModal").is(":visible") ) { //TODO: check if the change comes from a modal window.
						...
						if (objHtmlBindingRange && value !== "" &&
							value.replace(/\s+/i, " ") != " " && value != ";" && value != ":" && value != "," && value != "=" && value != ")" && value != "(" && value != "<" && value != ">") { //FIXME: regexp it!!
							
							// Modification will change a Binding fragment value
							...
							var endCols = objHtmlBindingRange.htmlBinding.attr("data-end-column").split(",")
							endCols[ objHtmlBindingRange.rangeIndex ] = objHtmlBindingRange.range.end.column

							objHtmlBindingRange.htmlBinding
									.attr("data-end-column", endCols.toString());
							objHtmlBindingRange.htmlBinding
									.text( document.editor.session.getTextRange( objHtmlBindingRange.range ) );
							
							DescriptionInspector.middleHighlightBindingExcept( objHtmlBindingRange.htmlBinding );
							DescriptionInspector.scrollInspectorToBinding(objHtmlBindingRange.htmlBinding, false);
							Binding.getBinding(objHtmlBindingRange.htmlBinding)
								.displaceRange(objHtmlBindingRange.range.start.row, objHtmlBindingRange.range.end.column, colIndex);

							DescriptionInspector.modifyBindings(
								objHtmlBindingRange.htmlBinding, document.editor.session.getTextRange(objHtmlBindingRange.range)
							);

							var binding = Binding.getBinding(objHtmlBindingRange.htmlBinding);
							
							setTimeout(function () {
								DescriptionInspector.setEditorAnnotation(binding.getAnnotations());
							}, 1200); // After checking syntax.

							console.log("Binding modified");
						
						} else if (actionType !== "removeLines" && actionType !== "insertLines") {
							// Modification wont change a Binding fragment value
							if (actionType === "insertText") {
								colIndex = value.length;
							} else if (actionType === "removeText") {
								colIndex = -value.length;
							}
							DescriptionInspector.displaceBindingRanges(range.start.row, range.end.column-(value.length+1), colIndex);
						}

						// Save description file content
						if (descriptionCheckerTimer && clearTimeout)
							clearTimeout(descriptionCheckerTimer);

						descriptionCheckerTimer = setTimeout(function () {
							DescriptionInspector.saveInspectorContentToDescriptionFile();
						}, 800);

					} else {
						// Modification comes from a modal window
						setTimeout(function () {DescriptionInspector.highlightAllBindings();}, 1200);
					}
		...
		loadInspectorTab : function (inspectorLoader) {
			inspectorLoader
				...
				.find(".descriptionInspectorTab").click(function () {
					console.log("Showing description inspector content.");

					$(".moduleInspectorContent").hide();
					$(".moduleInspectorTab")
						.removeClass("active")
						.find("a").css("background", "#fff");
					$(".moduleInspectorTab").css("border-bottom", "1px solid #EFEFEF");

					$(".descriptionInspectorContent").show();
					$(".descriptionInspectorTab")
								.addClass("active")
								.find("a").css("background", "rgba(0,0,0,0.02)");
		...
		onEditorAnnotationChange : function () {
			DescriptionInspector.ace.getSession().on("changeAnnotation", function () {
		...
		/**
		 * Muestra los cursores del editor si se selecciona el editor y se permite el cambio de contenido.
		 * @memberof DescriptionInspector.loaders
		 */
		onEditorSelectionShowCursors : function () {
			DescriptionInspector.ace.getSelection().on("changeCursor", function() {
				if (!DescriptionInspector.ace.getEditor().getReadOnly()) {
		...
		/**
		 * Realiza un scroll hasta el binding correspondiente haciendo clic sobre la línea resaltada de un Binding.
		 * @memberof DescriptionInspector.loaders
		 */
		onEditorBindingLineClick : function () {
			...
			function activateBindingFromLine() {
				...
				$(".ace_ideas_binding")
					.unbind("click")
					.click(function () {
						console.log("Activating Binding from row...");

						//TODO: activate binding lines from a row.

						var row = parseInt($(this).text())-1
							, bindingHTMLs = $bindingMap.getHTMLBindingsFromRow(row);
						...
						if (bindingHTMLs && bindingHTMLs.length > 0) {
							...
							for (var i=0; i < bindingHTMLs.length; i++) {
								var binding = Binding.getBinding(bindingHTMLs[i]);
								if (binding.isValid()) {
									ranges = ranges.concat(binding.getRanges());
									annotations = annotations.concat(binding.getAnnotations());

									if (i == 0) {
										DescriptionInspector.middleHighlightBindingExcept(bindingHTMLs[i]);
									} else {
										DescriptionInspector.highlightBinding(bindingHTMLs[i]);
									}
								}
							}

							if (ranges.length > 0) {
								DescriptionInspector.addRangesToEditor(ranges, annotations);
								DescriptionInspector.scrollInspectorToBinding(bindingHTMLs[0]);
							} else {
								console.log("No binding found to activate from line", row-1);
							}
						}
		...
		/**
		 * Da funcionalidad a todos los botones del menú contextual del inspector.
		 * @memberof DescriptionInspector.loaders
...
//			window.getSelection().empty();
		...
		/**
		 * Da funciondalidad al botón contextual "All bindings". Seleccionará y resaltará todos los bindings del inspector.
		 * @memberof DescriptionInspector.loaders
		 * @param {jQuery} inspectorLoader - Contenedor del inspector de descripción.
		 */
		...
		/**
		 * Da funcionalidad al botón contextual "Clear bindings". Quitará el resaltado de todos los bindigns.
		 * @memberof DescriptionInspector.loaders
		 * @param {jQuery} inspectorLoader - Contenedor del inspector de descripción.
		 */
		...
		/**
		 * Da funcionalidad al botón contextual "Zoom in". Incrementará el zoom del inspector al doble del existente.
		 * @memberof DescriptionInspector.loaders
		 * @param {jQuery} inspectorLoader - Contenedor del inspector de descripción.
		 */
		...
		/**
		 * Da funcionalidad al botón contextual "Zoom out". Disminuirá el zoom del contenido del inspector a la mitad del actual.
		 * @memberof DescriptionInspector.loaders
		 * @param {jQuery} inspectorLoader - Contenedor del inspector de descripción.
		 */
		...
		/**
		 * Realiza el ajuste de línea del contenido del inspector en función del valor actual del atributo "white-space".
		 * @memberof DescriptionInspector.loaders
		 * @param {jQuery} inspectorLoader - Contenedor del inspector de descripción.
		 */
		_wrappableWord : function (inspectorLoader) {
			var $inspector = $("#inspectorContent")
				, wrapText = "Wrap word"
				, unWrapText = "Unwrap word"
				;
		...
		/*
		 * Crea un MultiBinding a partir de otro Binding activo
		 * @memberof DescriptionInspector
		 */
		_multiBinding : function () {
			...
			$("#contextMenu .multiBinding")
				...
				.click(function (e) {
					...
					if (DescriptionInspector.isMultiBindingEnabled()) {
						var activeBindingHtmlObj = $(".ideas-binding, highlighted-fragment").not(".middle-highlighted-fragment")
							, range = document.editor.getSelectionRange().clone();

						if (activeBindingHtmlObj.length == 1 && !range.isEmpty()) {

							Binding.addRange(activeBindingHtmlObj, range);

							console.log("New range added to the selected binding.");
							...
							var binding = Binding.getBinding(activeBindingHtmlObj);
							$bindingMap.addBinding(binding);

						} else {
							console.log("Please, activate one binding for multiBinding.");
						}
					} else {
						console.log("MultiBinding is disabled. Please activate a Binding.");
					}
		...
		contextMenu : function (inspectorLoader) {
			...
			$("#inspectorContent")
				...
				.bind("contextmenu", function (e) {
					$("#contextMenu").css("position", "fixed"); // changing value to be able to view the contextmenu
					...
					$(document).click(function (e) {
						if (e.button != 2) {
							hideMenu();
							$("#contextMenu").css("position", "absolute"); // default value
						}
					});
				});
		...
		/**
		 * Cambia el valor de un binding en el fichero en el inspector y en el fichero que se le está haciendo binding.
		 * @memberof DescriptionInspector.loaders
		 * @param {jQuery} inspectorLoader - Contenedor del inspector de descripción.
		 */
		...
		/**
		 * Crea un fichero descriptor asociado al fichero actual de forma automática, y establece una plantilla.
		 * @memberof DescriptionInspector.loaders
		 * @param {jQuery} inspectorLoader - Contenedor del inspector de descripción.
		 */
		descriptionFileCreator : function (inspectorLoader) {
			inspectorLoader.append('\
				<article id="inspectorContent"><a class="btn descriptionFileCreator emptyMsg" style=" \
					color: #428bca; \
					top: 50%; \
					margin-top: 40%; \
				">Create a description file</a></article>');
			
			var $this = $("#editorInspector a.descriptionFileCreator");

			$this.css("margin-left", "-".concat( ($this.width()/2).toString() ));
			$this.css("margin-top", "-".concat( ($this.height()/2).toString() ));
			...
			$this.click(function () {
				...
				EditorManager.createNode(fileUri, fileName, ".html", function () {
					FileApi.saveFileContents(fileUri, fileContent, function () {
		...
		/**
		 * Da funcionalidad a los botones de para accionar los menúes de "Add" y "Remove".
		 * @memberof DescriptionInspector.loaders
		 * @param {jQuery} inspectorLoader - Contenedor del inspector de descripción.
		 */		
		expandableMenu : function (inspectorLoader) {
			...
			inspectorLoader
				.find(".inspectorButtonAdd")
				.click(function(event) {
					DescriptionInspector.expandInspectorMenu("add");
				});
		    
		    inspectorLoader
		    	.find(".inspectorButtonRemove")
		    	.click(function(event) {
		...
		/**
		 * Da funcionalidad a los bindings para que cuando se le haga clic seleccione el fragmento de texto asociado en ace.
		 * @memberof DescriptionInspector.loaders
		 * @param {jQuery} inspectorLoader - Contenedor del inspector de descripción.
		 */		
				...
				.click(function () {
					...
					if ($(this).find(".ideas-binding").length == 0 &&
						...
						if (binding.isValid()) {
							...
							DescriptionInspector.clearAllEditorMarkers();
							DescriptionInspector.ace.getEditor().navigateTo(ranges[0], -1);		// new make new mark always visible.
							for (var i=0; i < ranges.length; i++) {
							...
							DescriptionInspector.scrollEditorToRange(ranges[0]);

							DescriptionInspector.hideEditorCursors();

						} else {
							console.log("Couldn\'t highlight a invalid Binding", $(this))
						}

						console.log("Binding annotations", annotations);
	 ...
	 * @memberof DescriptionInspector
	 * @returns {boolean} Indica si el editor contiene alguna anotación de error de sintaxis.
	 */
	editorHasErrorAnnotation : function () {
		var actualAnnotations = document.editor.session.getAnnotations()
	...
	/**
	 * Carga sobre el editor las anotaciones pasadas por parámetro, sólo en el caso de que no exista
	 * ninguna anotación de error previamente.
	 * @param {array} annotations - Lista de anotaciones a cargar sobre el editor.
	 * @memberof DescriptionInspector
	 * @returns {boolean} Indica si se han cargado las modificaciones pasadas por parámetro.
	 */
	setEditorAnnotation : function (annotations) {
		var session = document.editor.session
			, ret 	= false;
		...
		if (!this.editorHasErrorAnnotation()) {
			...
			console.log("Couldn\'t set annotations because there is an systax error.");
		}
	...
	/**
	 * Desplaza los rangos de los bindings asociados a una fila, a partir de una columna determinada.
	 * @memberof DescriptionInspector
	 */
	displaceBindingRanges : function (row, fromColumn, n) {
		...
		if (row in $bindingMap.rowBindingsMap) {
			...
			bindingIds = $.unique($.unique(bindingIds));		//FIXME: double call to reorder.

			for (var i=0; i < bindingIds.length; i++) {
				var bindingId 			= bindingIds[i]
					, bindingHtmlObj 	= DescriptionInspector.getHTMLBinding(bindingId)
					, startColumns 		= bindingHtmlObj.attr("data-start-column").split(",")
					, endColumns 		= bindingHtmlObj.attr("data-end-column").split(",")
					, binding 			= Binding.getBinding(bindingHtmlObj);
				...
				while (pos < binding.startRows.length) {
					...
					if (index !== -1 && occurrencePositions.indexOf(index) === -1 && 
				...
				for (var pos=0; pos < occurrencePositions.length; pos++) {
					var startColumns = bindingHtmlObj.attr("data-start-column").split(",")
						, endColumns = bindingHtmlObj.attr("data-end-column").split(",")
						, occurrencePos = occurrencePositions[pos];

					startColumns[occurrencePos] = parseInt(startColumns[occurrencePos]) + parseInt(n);
					endColumns[occurrencePos] = parseInt(endColumns[occurrencePos]) + parseInt(n);

					bindingHtmlObj.attr("data-start-column", startColumns.toString());
	...
	/**
	/**
	 * Obtiene el html del Binding involucrado al rango que se le pasa por parámetro.
	 * @memberof DescriptionInspector
	 */
	getHtmlBindingFromRange : function (range) {
		...
		if (row in $bindingMap.rowBindingsMap) {
			...
			for (var i=0; i < bindingIds.length; i++) {
				var htmlBinding = DescriptionInspector.getHTMLBinding(bindingIds[i])
					, binding = Binding.getBinding(htmlBinding)
					, ranges = binding.getRanges();
				...
				for (var r=0; r < ranges.length; r++) {
					...
					if (range.start.row === rangeR.start.row &&
						...
						ret = {"htmlBinding": htmlBinding, "range":rangeR, "rangeIndex":r};
						break;
					}	
				}
	 ...
	 * @param {jQuery} htmlObj
	 * @param {string} value
	 * @memberof DescriptionInspector
	 */
	modifyBindings : function (htmlObj, value) {
		var binding  = Binding.getBinding(htmlObj)
			, ranges = binding.getRanges();
		...
		for (var i=0; i < ranges.length; i++) {
			...
			if (document.editor.session.getTextRange(range) !== value) {
				...
				var startCol 		= binding.startColumns[i]
					, finalEndCol 	= parseInt(startCol) + value.length
					, diffEndCol	= finalEndCol - range.end.column;
				...
				DescriptionInspector.ace.getSession().replace(
					range, ""
				);

				range.end.column = finalEndCol;

				var rngTxt = DescriptionInspector.ace.getSession().getTextRange(range);
				//console.log("rngTxt", rngTxt);
				DescriptionInspector.ace.getSession().replace(
					range, value.concat(rngTxt)
				);

				binding.displaceRange(binding.startRows[i], binding.endColumns[i], diffEndCol);
	...
	mayEnableMultiBinding : function () {
		return DescriptionInspector.vars.mode === "editor" && DescriptionInspector.isOneBindingActivated();
	},
	...
	isOneBindingActivated : function () {
		return $("#editorInspector .highlighted-fragment").length === 1 && $("#editorInspector .to-remove").length === 0;
	},
	...
	scrollEditorToRange : function (range) {
		var editor = DescriptionInspector.ace.getEditor();
		//editor.navigateTo(parseInt(range.start.row), parseInt(range.start.column) );
	...
	/**
	 * Transforma el texto seleccionado en un rango de selección sobre el editor.
	 * Sólo se realiza esta operación si no se trata de una selección múltiple.
	 * @memberof DescriptionInspector
	 */
	...
	clearAllEditorMarkers : function () {
		var session = DescriptionInspector.ace.getSession()
			, markerIds = Object.keys(session.getMarkers());
	...
	onEditorCheckedLanguage : function () {
		//console.log("Executing all editor checked language Description functions");
	...
	/**
	 * Muestra los demás binding con menos intensidad que el pasado por parámetro.
	 * Sirve para cuando hacemos clic sobre un binding específico y lo queremos resaltar más que los demás.
	 * @memberof DescriptionInspector 
	 */
	middleHighlightBindingExcept : function ($binding) {
		$("#editorInspector .ideas-binding").each(function () {
			if ( !$(this).is($binding) ) {
				//console.log(">>>>>>> middlehighlighting:", $(this), $(this).is($binding));

				$(this).removeClass("highlighted-fragment")
					   .addClass("middle-highlighted-fragment");
			} else {
				$(this).addClass("highlighted-fragment")
					   .removeClass("middle-highlighted-fragment");
			}
		});
	},
	
	/**
	 * Realiza "submit" de un formulario ubicado en la ventana modal, desde la sección de un campo de dicho formulario.
	 * @memberof DescriptionInspector 
	 */
	...
	scrollToFirstBinding : function () {
		DescriptionInspector.scrollInspectorToBinding(
			$(".highlighted-fragment").first()
		);
	},
	...
	scrollInspectorToBinding : function ($htmlObj, click) {
		if (DescriptionInspector.existBinding()) {
			...
			var topView = $("#inspectorContent").scrollTop()
				...
				, value = topView + topElement - 50
				, time = Math.abs(1500 - value * 0.4);

			if (time > 2000) time = 1500;		// if the inspectorContent is too large.
			...
			if ($("#editorInspector .to-remove").length === 0 && click) $htmlObj.click();
		}

	},

	/**
	 * Devuelve una lista de objetos jQuery con los bindings actuales en el inspector.
	 * Los Bindings disponibles son aquellos que se pueden activar o borrar.
	 * @memberof DescriptionInspector
	 * @returns {jQuery}
	 */
    ...
    highlightBinding : function ($obj) {
		$obj.addClass("highlighted-fragment")
			.removeClass("middle-highlighted-fragment");
    },
    ...
    highlightAllBindings : function () {
		...
		if (DescriptionInspector.existBinding()) {
			...
			DescriptionInspector.ace.getEditor()
				.navigateTo(document.editor.getSelectionRange().start.row, -1);

			$("#editorInspector").find(".ideas-binding, .to-remove").each(function () {
	
				var binding = Binding.getBinding( $(this) );

				if (binding.isValid()) {
					annotations = (annotations.length == 0) ? 
						binding.getAnnotations() : annotations.concat(binding.getAnnotations());
			...
			if (DescriptionInspector.vars.mode == "editor" && DescriptionInspector.isFirstEditorFormatSelected()) {
				...
				if (ranges.length > 0) DescriptionInspector.scrollEditorToRange(ranges[ranges.length-1]);
				
				DescriptionInspector.setEditorAnnotation(annotations);
				DescriptionInspector.ace.getSelection().clearSelection();

				if ($("#editorInspector .to-remove").length === 0) DescriptionInspector.loaders.bindings();
     ...
     * @memberof DescriptionInspector
	 * @param {jQuery} $obj - Representación de Binding en HTML.
     */
	...
	/**
	 * Muestra u oculta el menu expandible del inspector. Para ello, se le pasa un valor como parámetro que indicará si se debe mostrar o no el menú.
	 * @memberof DescriptionInspector
	 * @param {boolean} show - Se muestra si es verdadero, en caso contrario, se oculta.
	 */
...
/**
 * Representa un binding. Un binding debe tener una anotación asociada, fila inicial y final, y columna inicial y final.
 * @namespace Binding
 * @constructor
 * @param {number} id - Identificador de Binding.
 * @param {string} textNote - Anotación del binding.
 * @param {array} startRows - Filas inicial del binding.
 * @param {array} endRows - Filas final del binding.
 * @param {array} startColumns - Columnas inicial del binding.
 * @param {array} endColumns - Columnas final del binding.
 */
function Binding(id, textNote, startRows, endRows, startColumns, endColumns) {
	...
	this.startRows 		= (types.indexOf(typeof startRows) !== -1) ? [startRows] : startRows;
	this.endRows 		= (types.indexOf(typeof endRows) !== -1) ? [endRows] : endRows;
	this.startColumns 	= (types.indexOf(typeof startColumns) !== -1) ? [startColumns] : startColumns;
	this.endColumns 	= (types.indexOf(typeof endColumns) !== -1) ? [endColumns] : endColumns;
...
/*
 * Obtiene el ID asociado a un Binding a partir de su objeto HTML.
 * @memberof Binding
 */
...
/**
 * Devuelve un objeto Binding a partir de un objeto html. Para ello, se saca toda la información necesaria a partir de los atributos "data" del parámetro.
 * @memberof Binding
 * @param {jQuery} htmlObj
 */
Binding.getBinding = function (htmlObj) {
	//console.log("ejecutando getBinding", htmlObj.data("start-row"), typeof htmlObj.data("start-row"))
	return new Binding(
		Binding.getBindingID(htmlObj),
		DescriptionInspector.getBindingText(htmlObj),
	  	htmlObj.attr("data-start-row").toString().split(","),
	   	htmlObj.attr("data-end-row").toString().split(","),
	   	htmlObj.attr("data-start-column").toString().split(","),
	   	htmlObj.attr("data-end-column").toString().split(",")
	);
};

/**
 * Comprueba si la instancia contiene todos los valores correctamente inicializados.
 * @memberof Binding
 */
Binding.prototype.isValid = function () {
	...
	return typeof this.textNote != "undefined" 		&& this.textNote != "" &&
		   typeof this.startRows != "undefined" 	&& this.startRows.toString().split(",").length >= 0 &&
		   typeof this.endRows != "undefined" 		&& this.endRows.toString().split(",").length >= 0 &&
		   typeof this.startColumns != "undefined" 	&& this.startColumns.toString().split(",").length >= 0 &&
		   typeof this.endColumns != "undefined" 	&& this.endColumns.toString().split(",").length >= 0;
};
...
Binding.prototype.getAnnotations = function () {
	...
	for (var i=0; i < this.startRows.length; i++) {
		...
		if ( selectionNumLines == 1 ) {
			// One-line annotation
			annotations.push({ row : this.startRows[i], 
				type : "binding", 
				text : this.textNote });
		} else {
...
Binding.prototype.getRange = function () {
	...
	range.setStart({row: this.startRows[0], column: this.startColumns[0]});
	range.setEnd({row: this.endRows[0], column: this.endColumns[0]});
...
Binding.prototype.getRanges = function () {
	...
	for (var i=0; i < this.startRows.length; i++) {
		...
		range.setStart({row: this.startRows[i], column: this.startColumns[i]});
		range.setEnd({row: this.endRows[i], column: this.endColumns[i]});

		ranges = (ranges.length === 0) ? [].concat(range) : ranges.concat(range);
	}
...
/**
 * Obtiene los rangos del binding que pertenezcan a una fila inicial dada omitiendo el del objeto instanciado.
 * @memberof Binding
 */
Binding.prototype.getOmmitedRowRanges = function (row) {
	var distinctBindingIds = []
		, _thisId = this.id
		, ranges = [];

	if (row in $bindingMap.rowBindingsMap) {

		var distinctBindingIds = $bindingMap.rowBindingsMap[row].filter(function (elto) {
			return elto != _thisId;
		});
		...
		for (var i=0; i < distinctBindingIds.length; i++) {
			var bindingId = distinctBindingIds[i]
				, binding = Binding.getBinding( DescriptionInspector.getHTMLBinding(bindingId) );
...
Binding.prototype.getTargetText = function () {
	...
	return DescriptionInspector.ace.getSession().getTextRange( this.getRange() );
};
...
Binding.prototype.getRows = function () {
	...
	for (var i=0; i < this.startRows.length; i++) {
		var startRow = this.startRows[i]
			, endRow = this.endRows[i];
...
Binding.addRange = function (bindingHtmlObj, range) {
	...
	if (!range.isEmpty()) {
		...
		var binding = Binding.getBinding(bindingHtmlObj)
			, newStartRow = range.start.row, newEndRow = range.end.row
			, newStartColumn = range.start.column, newEndColumn = range.end.column;

		if (binding.isValid() && !binding.hasRange(range)) {

			// Modifies original Binding
			bindingHtmlObj.attr("data-start-row", 	 bindingHtmlObj.attr("data-start-row").concat(","+ newStartRow));
			bindingHtmlObj.attr("data-end-row", 	 bindingHtmlObj.attr("data-end-row").concat(","+ newEndRow));
			bindingHtmlObj.attr("data-start-column", bindingHtmlObj.attr("data-start-column").concat(","+ newStartColumn));
			bindingHtmlObj.attr("data-end-column",   bindingHtmlObj.attr("data-end-column").concat(","+ newEndColumn));
...
Binding.prototype.getBindingHtml = function () {
	return DescriptionInspector.getHTMLBinding(this.id);		//FIXME: desacoplar de DescriptionInspector
};

/**
 * Desplaza los rangos de una línea n caracteres hacia la derecha, excepto el objeto instanciado.
 * @memberof Binding
 */
Binding.prototype.displaceRange = function (row, thisEndColumn, n) {
	...
	if (row in $bindingMap.rowBindingsMap) {
		...
		distinctBindingIds = $.unique($.unique(distinctBindingIds));		//FIXME: double call to reorder.
...
/**
 * Comprueba si el id del Binding pasado por parámetro existe dentro de BindingMap
 * @param {Binding} bindingObj
...
/**
 * Carga sobre el objeto el mapeo de todos los Bindings disponibles en el sistema.
 * @memberof BindingMap
 */
BindingMap.prototype.fetchBindings = function () {
	...
	if (DescriptionInspector.vars.mode === "editor") {
		...
		$("#editorInspector").find(".ideas-binding, .to-remove").each(function () {
			//TODO: si no se encuentra dentro del inicializado, cargalo con el ID del objeto
			if (($(this).attr("id") && $(this).attr("id").search(/ideasBinding_/i) === -1) || 
				...
				console.log("Automatically assigned BindingId", newBindingId);
				b = true;
			}
...
BindingMap.prototype.addBinding = function (bindingObj) {
	...
	if (this.bindingRowsMap[bindingObj.id] === undefined) {
		this.bindingRowsMap[bindingObj.id] = BindingMap.getBindingRowsStructure();
	}
...
BindingMap.prototype.getHTMLBindingsFromRow = function (row) {
	...
	if (row in this.rowBindingsMap) {
		var bindingIds = this.rowBindingsMap[row]
			, array = [];
		for (var i=0; i < bindingIds.length; i++) {
			var e = $("#ideasBinding_"+ bindingIds[i]);
			if (e && array.indexOf(e) === -1) array.push(e);
		}
