...
var setupLanguages = function(configuration) {
	for (var languageId in configuration.languages) {
		var uri = configuration.languages[languageId];
		ModeManager.idUriMap[languageId] = uri;
		console.log("Setting up language: " + languageId + " (" + configuration.languages[languageId] + ")");
		configureLanguage(uri);
	}
};

var configureLanguage = function(uri) {
	$.ajax(uri + "/language", {
		"type" : "get",
		...
		"error" : function(result) {
			console.error("Could not load language from " + uri +": "+ result.statusText);
		},
		"async" : true,
	});
};
