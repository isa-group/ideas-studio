angular.module('mainApp')
    .factory('stateService', function () {
        return function (changesToState) {
            ...
            angular.forEach(changesToState, function (value, key) {
                if (key == "genericState" && value == "START")
                    stateName = "active"
                else if (key == "genericState" && value == "END")
                    stateName = "completed"
                else if (key == "dataObjectState")
        ...
        return {
            ...
            link: function (scope, element, attrs) {
                if (scope.baseMeasure != null) {
                    $compile("<span>the <span contenteditable ng-model='aggFunction'></span> of <measure ng-model='baseMeasure' process='process'></measure></span>")(scope, function (cloned, scope) {
    ...
    .directive('derivedmeasure', function ($compile) {
        return {
            ...
            link: function (scope, element, attrs) {
                var template = '<span> <em><span contenteditable ng-model="derived.function"></span></em>, where: <ul>' +
                    '<li ng-repeat="(v, def) in derived.usedMeasureMap">' +
                    '{{v}} is <measure ng-model="derived.usedMeasureMap[v]" process="process"></measure>' +
                    '</li>' +
                    '</ul></span>';
    ...
    .directive('event', ['stateService', function ($stateService) {
            return {
                ...
                transclude: true,
                scope: {ngModel: '=', process: '='},
                template: '<span>{{process.id[ngModel.appliesTo].type}} <em>{{process.id[ngModel.appliesTo].name}}</em> becomes ' +
